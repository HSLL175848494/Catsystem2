...
struct InI//用于储存配置文件结构体
{
	...
	SegmentCptype cptype;//cptype段(可能是compress type即压缩类型的缩写)
	...
	if (mode == 4)
	{
		...
		do
		{
			if (FindData.dwFileAttributes == FILE_ATTRIBUTE_DIRECTORY && hfind != INVALID_HANDLE_VALUE)//过滤非文件夹
			{
				...
				{
					WCHAR filenameini[64]{};//用于储存ini文件名
					...
					{
						WCHAR hg3name[128]{};//用于记录生成的hg3图片名
						//填入文件名
					...
					do
					{
						...
						{
							WCHAR pngname[128]{};//用于记录每个ini文件对应的png图片名（现在临时记录为ini的文件名）
							...
							if (data != nullptr)
							{
								...
								while (1)
								{
									...
									if (!lstrcmpA(segmenthead.Lable, "stdinfo"))
									{
										...
										//查找是否还有下一张图片
										DWORD namelength = lstrlenW(pngname);
										pngname[namelength - 5] += 1;
										for (int i =  namelength- 5; i > namelength - 8; i--)
										{
											if (pngname[i] > '9')
											{
												pngname[i] = '0';
										...
										//记录块偏移(没有下一个块则偏移为0)
										infomation.BlockOffset = GetFileAttributesW(pngname)== INVALID_FILE_ATTRIBUTES?0:GetFileSize(hini, nullptr) + img0000.dCompressLength + img0000.mCompressLength + 44;
