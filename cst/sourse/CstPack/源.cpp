...
int main()
{
	...
	do
	{
		...
		if (FindData.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			if (TxtSize)
			{
				...
				//替换特殊字符例如※＠
				WORD* spcChar = (WORD*)dTxt;
				for (unsigned int i = 0; i < TxtSize - 1; i++)
				{
					if (*spcChar == 0xf9a1)
						*spcChar = 0xa681;
					else if (*spcChar == 0xc0a3)
						*spcChar = 0x9781;
					spcChar = (WORD*)((char*)spcChar + 1);
				}
			}
			...
			while (HashRead < IniSize)
			{
				...
				*dPtr = *Command; Command++;
				...
				//只有0x30指令中实际为标题或者选择支时才会在导出时修正为0xff，因此0xff部分内容需要汉化且已经装入到txt内
				//判断是否为需要汉化的内容
				...
				if (IfScene)//是需要汉化的内容，具体指令内容从txt内容对应数组读取
				{
					...
					while (true)
					{
						...
						if (*Scene == '\n')//达到结束时（以0标识结束）
						{
							...
							// 还原特定控制字符@为日文编码(用于控制人名的隐藏替换)
							if (SceneSize >= 1)
							{
								for (int i = 0; i < SceneSize - 1; ++i)
								{
									if (dPtr[i] == 0xa3 && dPtr[i + 1] == 0xc0)
									{
										dPtr[i] = 0x81;
										dPtr[i + 1] = 0x97;
										++i;
	...
	WriteConsoleA(GetStdHandle(STD_OUTPUT_HANDLE), "剧本导入完成，将在3s后自动关闭", strlen("剧本提取完成，将在3s后自动关闭"), nullptr, nullptr);
	Sleep(3000);
	return 0;
}
