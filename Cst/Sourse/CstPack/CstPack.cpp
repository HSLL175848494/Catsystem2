...
int main()
{
	...
	do
	{
		...
		if (FindData.dwFileAttributes != FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			{
				//获取指令类型
				*dPtr = *Comment; Comment++;
				
				//只有0x30指令中实际为标题或者选择支时才会在导出时修正为0xff，因此0xff需要汉化且内容是已经装入到txt内
				//判断是否为需要汉化的内容
				bool IfScene = (*Comment ==0xff|| *Comment==0x20|| *Comment==0x21);

				//还原0xff指令为0x30
				*(dPtr+1) =(*Comment==0xff?0x30: *Comment);	Comment++;
				...
				else 	//非剧本指令，具体指令内容从ini文件内读取
				{
					...
					{
						CommentSize++;
						if (!(*Comment))//达到结束时（以0标识结束）
						{
							Comment++;
							//记录非剧本指令
							memcpy(dPtr, Comment - CommentSize, CommentSize);

							//修正长偏移表的偏移位置
							memcpy(tPtr, &tOffset, 4);
							tOffset += CommentSize + 2;

							//移动指针
							tPtr += 4;
							dPtr += CommentSize;
							HashRead += CommentSize;
