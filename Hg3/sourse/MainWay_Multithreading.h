...
void reform(BYTE* data, DWORD length, DWORD height, BYTE*& datatable, BYTE*& maptable, DWORD& mUncompresslength, DWORD& dUncompresslength, DWORD& mCompresslength, DWORD& dCompresslength)
{
	...
	datatable = ZlibCompress(uncompresseddata, dCompresslength);//压缩数据
	maptable = ZlibCompress(uncompressedmap, mCompresslength);//压缩数据
}
DWORD WINAPI TaskThreadProc1(LPVOID lparameter)
{
	Task task = *(Task*)lparameter;
	HANDLE hsemaphore = task.hsemaphore;
	WIN32_FIND_DATAW FindData = task.FindData;
	HANDLE hfind = task.hfind;
	...
	delete (Task*)lparameter;
	ReleaseSemaphore(hsemaphore, 1, nullptr);
	ExitThread(0);
}

DWORD WINAPI TaskThreadProc2(LPVOID lparameter)
{
	Task task = *(Task*)lparameter;
	HANDLE hsemaphore = task.hsemaphore;
	WIN32_FIND_DATAW FindData = task.FindData;
		...
		while (1)//读取每个块的数据
		{
			...
			while (1)
			{
				...
				else if (!lstrcmpA(segmenthead.Lable, "img0000"))//内置的一种图片格式
				{
					...
					img0000.Data = ZlibUncompress(img0000.Data, img0000.dCompressLength);//解压后的数据体
					img0000.Map = ZlibUncompress(img0000.Map, img0000.mCompressLength);//解压后的数据表
				...
				else if (!lstrcmpA(segmenthead.Lable, "img_al"))//记录透明通道，如果有jpg数据，即可进行图片转化
				{
					...
					if (img_jpg.JpgData)//如果不是空值
						alpha = ZlibUncompress(img_al.Alpha, img_al.CompressLength);//解压透明通道
...
bool CreateTask1(HANDLE hmutex, WIN32_FIND_DATAW FindData, HANDLE hfind)
{
	if (hmutex == INVALID_HANDLE_VALUE)
		return false;
	WaitForSingleObject(hmutex, INFINITE);
	Task* task = new Task();
	*task = { hmutex,FindData,hfind };
	HANDLE hthread = CreateThread(nullptr, 0, TaskThreadProc1, task, 0, nullptr);
	CloseHandle(hthread);
	return true;
}

bool CreateTask2(HANDLE hmutex, WIN32_FIND_DATAW FindData, HANDLE hfind)
{
	if (hmutex == INVALID_HANDLE_VALUE)
		return false;
	WaitForSingleObject(hmutex, INFINITE);
	Task* task = new Task();
	*task = { hmutex,FindData,hfind };
	HANDLE hthread = CreateThread(nullptr, 0, TaskThreadProc2, task, 0, nullptr);
	CloseHandle(hthread);
	return true;
}
...
void trans_hg3()
{
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////reform(?--->hg3)
	WIN32_FIND_DATAW FindData;
	HANDLE hsemaphore = CreateSemaphoreW(nullptr, 0, 1024, nullptr);
	DWORD semaphorecount = 0;
	if (mode == 4)
	{
		...
		do
		{
			ReleaseSemaphore(hsemaphore, 1, nullptr);
			semaphorecount++;
			CreateTask1(hsemaphore, FindData, hfind);
	...
	if (hfind != INVALID_HANDLE_VALUE)
	{
		do
		{
			ReleaseSemaphore(hsemaphore, 1, nullptr);
			semaphorecount++;
			CreateTask2(hsemaphore, FindData,hfind);
		} while (FindNextFileW(hfind, &FindData));//搜索下一个后缀为.hg3的文件
		FindClose(hfind);
		for (int i = 0; i < semaphorecount; i++)
		WaitForSingleObject(hsemaphore, INFINITE);
