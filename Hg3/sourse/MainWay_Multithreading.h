...
BYTE* operate(char* data, DWORD height, DWORD length)//差分表还原为原数据（差分表的数据记录的是与它相邻像素的差值）
{
	...
	for (int i = 0; i < height; i++)//截取差分表中长为length*height*4的一段
	{
		...
		memcpy(&array[i][0], data + i * wide, wide);
	}
	...
	for (int i = 0; i < wide; i++)//第二行直接求前缀和，反正都是不同的像素，值可以直接相加
	{
		for (int k = 1; k < height; k++)
			array[k][i] += array[k - 1][i];
	}
	BYTE* result = new BYTE[height*wide]();
	for (int i = 0; i < height; i++)//拷贝还原的值
	{
		memcpy(result + i * wide, &array[i][0],wide);
		delete[]array[i];
	}
	delete[]array;
	return result;
...
char* reoperate(char* data, DWORD height, DWORD length)//将原像素值还原为差分表，这是像素的差分记值法。过程与上面差分表转像素相反（由于像素差值可能为负，将其转为有符号类型的char*）
{
	...
	char* result = new char[length * wide > 0x400 ? wide * height : 0x400]();
	for (int i = 0; i < height; i++)
	{
		memcpy(result + i * wide, &array[i][0], wide);
		delete[]array[i];
...
{
	DWORD size = height * length * 4;
	DWORD rightsize = size> 0x400 ?size : 0x400;
